...
include('header.php'); ?>
	<div id="contenido">
		...
		<?php 
			$ch = curl_init(); 
			curl_setopt($ch, CURLOPT_URL, URL_API."zonas/".$_GET['id']); 
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1); 
			$output = json_decode(curl_exec($ch)); 
			$id_estado = $output->id_estado;
			$nombre = $output->nombre;
			curl_close($ch); 
		?>
			<form class='validation-form'  action="api/update/zonas" method="post" enctype="multipart/form-data">
				<div class="form-group">
					<select name="id_estado" id="" style="width: 100%">
						...
						<?php 
							...
							foreach($output as $o){
								?>
						<option value="<?php echo $o->id ?>" <?php if($o->id == $id_estado){ echo ' selected ';} ?>><?php echo $o->nombre ?></option>
								<?php
							}
						?>
					</select>
